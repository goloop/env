...
| `LoadSafe(filenames ...string) error`    | `${VAR}` розгортається | зберігаються (пропускає відсутній файл) |
...
`Load` помиляється, якщо названий файл відсутній; `LoadSafe` пропускає відсутній
файл (але все одно повертає помилки парсингу чи вводу-виводу) - це патерн
«підвантаж якщо є»: читай `.env` у розробці, а в CI/проді покладайся на реальне
оточення.
...
| Тег | Опис |
|-----|------|
| `env` | Ім'я ключа. `-` повністю ігнорує поле. Inline-прапорці йдуть після імені через кому: `required`, `absolute` (напр. `env:"KEY,required"`). |
...
`env:"NAME,absolute"` робить ім'я ключа повним іменем змінної оточення,
ігноруючи префікс, який додали б охоплюючі структури, - включно з заданим через
`WithPrefix`. Деплой фіксує деякі імена раз і назавжди, і групування полів у Go
не має їх перейменовувати:

```go
type Config struct {
	DB struct {
		URL      string `env:"DATABASE_URL,absolute"` // читає DATABASE_URL
		PoolSize int    `env:"POOL_SIZE"`             // читає DB_POOL_SIZE
	} `env:"DB"`
}
```

Скидається весь ланцюжок префіксів, не один рівень, а `Marshal` пише те саме
ім'я, яке читає `Unmarshal`. Поєднується з `required`.
...
| `Expand(value) string`       | `${VAR}`/`$VAR` з оточення |
...
**Посилання є посиланням лише тоді, коли воно називає ключ.** `VAR` має
відповідати `[A-Za-z_][A-Za-z0-9_]*` - тим самим іменам, які цей формат уміє
визначати. Усе інше зберігає свій `$` точно як написано:

```ini
PRICE=cost: $100      # -> cost: $100
AWK=$1 == "x"         # -> $1 == "x"
SED=s/a/b/$2          # -> s/a/b/$2
PASSWORD=pa$$word     # -> pa$$word , подвоєний $ ніколи не є посиланням
TRAILING=100$         # -> 100$
```

Власного екранування для `$` немає, і воно не потрібне: одинарні лапки й
бектики - це літеральні форми, які цей формат уже має.

Посилання на незадане ім'я стає порожнім, як і було. Незакрите `${` - це
текст, а не вказівка з'їсти решту значення.

Варіанти `Raw`/`ParseRaw` повністю вимикають розгортання.

## Рецепти й поради

### Валідація того, що декодовано

Канонічний завантажувач - три кроки, і третій ваш:
...
func Load(files ...string) (*Config, error) {
    ...
    if err := c.Validate(); err != nil { // крос-польові правила тут
        return nil, err
    }
    return &c, nil
}
...
Прапор `required` покриває наявність; правила на кілька полів - два секрети
мусять різнитись, ліміт має бути нижчим за стелю, ключ достатньо довгим для
підпису - тегом не виражаються й належать методу `Validate`, який застосунок
пише сам і кличе на старті: місконфігурація тоді відмовляється стартувати, а
не падає на першому запиті, якому знадобилось значення. `Unmarshal` свідомо
не викликає такий метод сам: забути написати метод так само легко, як забути
виклик, а декодер, що виконує логіку застосунку, - дивна річ у дебазі.
