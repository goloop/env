...
- Тег `def` дає значення, що вживається за відсутності ключа.
...
|-----------|------|
| Рядки     | `string` |
| Булеві    | `bool` (`true`/`false`, також `1`/`0`, `t`/`f`, … за `strconv.ParseBool`) |
...
KEY=value   # inline-коментар
NOTE="символ # у лапках є частиною значення"
```

`#` починає коментар лише поза лапками. Якщо значення містить `#`, візьми його
в лапки.
