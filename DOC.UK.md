...
`WithSeparator`, дефолт — кома) і декодується розбивкою по ньому. Щоб список
round-trip-ився, **елемент не повинен містити роздільник** (чи лапки). Обери
`sep`, якого нема у твоїх значеннях, або змоделюй складний елемент типом, що
реалізує `encoding.TextMarshaler`/`encoding.TextUnmarshaler`.

При записі у файл `MarshalFile` бере в лапки скалярні значення, які інакше
прочитались би невірно (newline, край-пробіли, inline-`#`), тож файл
round-trip-иться через `UnmarshalFile`.
